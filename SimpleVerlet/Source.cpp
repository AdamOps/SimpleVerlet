...
int main()
{
    ...
    while (window.isOpen())
    {
        ...
        for (int iter = 0; iter < constrainIterations; iter++) {
            for (int i = 0; i < objectSet.size(); i++) {
                objectSet.at(i).constrainPoints(&window);
            }

            // Only update the sticks if they're active.
            if (drawSticks == true) {
        ...
        for (int i = 0; i < objectSet.size(); i++) {
            objectSet.at(i).shape.setPosition(objectSet.at(i).position);
